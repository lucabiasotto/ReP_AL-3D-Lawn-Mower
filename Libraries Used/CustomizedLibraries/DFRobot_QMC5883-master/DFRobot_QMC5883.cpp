...
Vector DFRobot_QMC5883::readNormalize(void)
{
  ...
  if(isHMC_){
    ...
    //TODO modifica il metodo readRegister16 in modo che accetti un secondo parametro (keepConnectionOpen)
    // in quest modo posso eliminare quei delay tra le richieste, le richieste vengono fatte tutte nella stessa trasmissione
    //apro la trasmissione, ciclo e leggo, chiudo trasmissione dopo il while...così posso elminare quei delay
    //fatto questo ripristina il delay in Compass.Get_Compass_Reading

    while (range--){

      XAxis = (float)readRegister16(QMC5883_REG_OUT_X_M);
      v.XAxis = XAxis * mgPerDigit;

delay(END_TRASMISSION_DELAY); //[LB]prevent endTransmission hangs

      YAxis = (float)readRegister16(QMC5883_REG_OUT_Y_M);
      v.YAxis = YAxis * mgPerDigit;

delay(END_TRASMISSION_DELAY); //[LB]prevent endTransmission hangs

      ZAxis = (float)readRegister16(QMC5883_REG_OUT_Z_M);
