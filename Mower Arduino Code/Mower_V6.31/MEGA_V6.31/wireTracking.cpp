...
void PrintWirePosition() {
    ...
    robot.printInMax = InMax / Scale;
    robot.printInMid = InMid / Scale;
    robot.printInMin = InMin / Scale;
    robot.printOutMin = OutMin / Scale;
    robot.printOutMid = OutMid / Scale;
    robot.printOutMax = OutMax / Scale;
        ...
        if (robot.mowerParked == 0) {
            ...
            if (CW_TRACKING_EXITING_CHARGE == 0) {
                if (robot.magError > 0) {  // If the robot.magError > 0 then turn right for CCW Tracking. PWM_left is set to max to turn right.
                    // TURN RIGHT
                    robot.pwmLeft = PWM_MaxSpeed_LH;            // sets the PWM to the max possible for the wheel
