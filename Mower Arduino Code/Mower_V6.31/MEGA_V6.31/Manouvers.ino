...
void Manouver_Mow_The_Grass() {
    ...
    if (Loop_Cycle_Mowing == 5) {
        ...
        if ((Compass_Heading_Hold_Enabled == 1) && (Compass_Activate == 1)) {
            ...
            if (Pattern_Mow == 1)
                Heading_Lock = Compass_Target;  // If pattern mow is enabled follow that heading
            if (Pattern_Mow == 0)
                Heading_Lock = Compass_Heading_Degrees;  // Otherwise use the current heading as the lock
...
void findWire() {
    ...
    for (int i = 0; i <= 1; i++) {  //che cazzo è sta merda??? deve fare 2 volte la stessa cosa
        ...
        if (Mower_Parked == 0) {
            ...
            if ((inside == false) && (Abort_Wire_Find == 0)) {
            ...
            if (inside == true && Abort_Wire_Find == 0 && No_Wire_Found == 0) {
...
void Manouver_Turn_Around() {
    ...
    if ((Pattern_Mow == 1) && (Compass_Activate == 1)) {
        ...
        if (Compass_Leg == 1) {
            ...
            Compass_Target = Compass_Mow_Direction + 90;
            if (Compass_Target < 0)
                (Compass_Target = 360 + Compass_Target);
            Serial.print("|Target Degree Heading = ");
            Serial.print(Compass_Target);
            Turn_To_Compass_Heading();
...
void goToChargingStation() {
    Mower_Docked = 0;
    Mower_Parked = 0;  //<-- DIVENTA 1 quando arriva alla base di ricarica
    Mower_Running = 0;
    Mower_Parked_Low_Batt = 0;
    ...
    // Switches on or off the Alarms depending on the setup
    Manage_Alarms();
    ...
    if (Mower_Parked == 0 && Compass_Activate == 1) {
        //TODO.....approfondire
    ...
    //3. segue il filo
    if (Mower_Parked == 0 && No_Wire_Found == 0) {
        followingWireToDock();
    }
