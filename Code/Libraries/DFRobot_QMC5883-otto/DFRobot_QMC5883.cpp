...
//[LB] my variable
int READ_ERROR = -10000;
int END_TRASMISSION_DELAY = 3; //5 is ok
...
Vector DFRobot_QMC5883::readNormalize(void)
{
  ...
  float Zsum = 0.0;
  if(isHMC_){
    beginDFTransmission();//[LB] open transmission

    while (range--){
      v.XAxis = ((float)readRegister16OnTransmission(HMC5883L_REG_OUT_X_M )) * mgPerDigit;
      v.YAxis = ((float)readRegister16OnTransmission(HMC5883L_REG_OUT_Y_M )) * mgPerDigit;
      v.ZAxis = (float)readRegister16OnTransmission(HMC5883L_REG_OUT_Z_M) * mgPerDigit;
      Xsum += v.XAxis;
      Ysum += v.YAxis;
      Zsum += v.ZAxis;
    }
    endDFTransmission();//[LB] close transmission
    ...
    while (range--){
      ...
      XAxis = (float)readRegister16OnTransmission(QMC5883_REG_OUT_X_M);
      v.XAxis = XAxis * mgPerDigit;

      YAxis = (float)readRegister16OnTransmission(QMC5883_REG_OUT_Y_M);
      v.YAxis = YAxis * mgPerDigit;

      ZAxis = (float)readRegister16OnTransmission(QMC5883_REG_OUT_Z_M);
      v.ZAxis = ZAxis * mgPerDigit;

      if(XAxis == READ_ERROR || YAxis == READ_ERROR || ZAxis == READ_ERROR){
        Serial.println("COMPASS_READ_ERROR");
        Xsum += Xsum/range;
        Ysum += Ysum/range;;
        Zsum += Ysum/range;;
      }else{
...
int16_t DFRobot_QMC5883::readRegister16(uint8_t reg)
{
  ...
  if(isHMC_){
    ...
    Wire.requestFrom(QMC5883_ADDRESS, 2, true); //[LB] true: closse connection

    int notAvailableCount = 10;
    while(!Wire.available()) {
      notAvailableCount--;
      Serial.print("|WIRE_NOT_AV");
    ...
    //byte error = Wire.endTransmission(); /[LB] non serve a un cazzo chiudo con requestFrom
...
/**
* Read register using open Transmission
* you have to open and close it mannually
*/
int16_t DFRobot_QMC5883::readRegister16OnTransmission(uint8_t reg)
{
  int16_t value=0;
  if(isHMC_){
        ...
        Wire.send(reg);
    #endif
    Wire.endTransmission(false); //[LB] false: keep connection open, only commit
    
    Wire.requestFrom(HMC5883L_ADDRESS, 2, false); //[LB] false: not closse connection
    int notAvailableCount = 10;
...
void DFRobot_QMC5883::endDFTransmission(void){
  Wire.endTransmission();
}
